...
export default function TrechoOrigem({ trechoId, afirmacao }: Props) {
  ...
  if (estado.fase === "erro") {
    return (
      <p className="mt-3 text-xs text-slate-600 italic dark:text-slate-400">
  ...
  return (
    <div className="mt-3">
      <div className="flex items-center justify-between gap-3">
        ...
        <button
          onClick={onFechar}
          className="alvo-toque -my-2 -mr-2 rounded-lg px-2 text-xs text-slate-600 underline-offset-2 hover:underline dark:text-slate-400"
        >
          Fechar
        </button>
      </div>
      ...
      <p className="mt-1.5 text-xs text-slate-600 italic dark:text-slate-400">
        {passagem
